...
import numpy as np
import torch
from utils import run_game,is_winner
from model import Hyperparams
from players import AITrainer, BluffPlayer, CallPlayer, RandomPlayer, linear_schedule
    ...
    def load_agents(self):
        ...
        if self.load_snapshot and len(filenames) > 0:
            for filename in filenames:
                ...
                if not is_frozen:
                    if main_loaded: continue
                    print(f'main agent loaded from snapshot: {filename}')
                    self.main_agents = []
